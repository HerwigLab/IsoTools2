...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="prepare_data", description="subset genomic data for testing"
    )
    parser.add_argument(
        "--annotation",
        metavar="<file.gff.gz>",
        help="specify reference annotation [requires tabix index]",
    )
    parser.add_argument(
        "--genome",
        metavar="<genome.fasta>",
        help="specify reference genome file [requires fai index]",
    )
    parser.add_argument(
        "--alignment",
        metavar="<alignment.bam>",
        help="specify alignment bam file [requires bai index]",
    )
    parser.add_argument(
        "--subsample_alignment",
        metavar="<fraction>",
        help="subsample the alignment file",
        type=proportion,
    )
    parser.add_argument(
        "--regions",
        metavar="<chr:start-end[,chr:start,end[,...]]>",
        default="chr2:214700000-216200000,chr8:22000000-23000000",
        help="regions to subset",
    )
    parser.add_argument(
        "--out_prefix",
    ...
    regions = [
        (regstr[: regstr.find(":")],)
        + tuple(int(i) for i in regstr[regstr.find(":") + 1 :].split("-"))
        for regstr in args.regions.split(",")
    ]
    # get subsets
    ...
    if args.alignment:
        subset_alignment(
            ...
            regions,
            args.out_prefix + ".bam",
            proportion=args.subsample_alignment,
        )
    if args.domains:
...
def subset_alignment(bam_fn, regions, out_fn="example.bam", proportion=None):
    header = {
        "HD": {"VN": "1.0"},
        "SQ": [
            {"LN": end - start, "SN": chrom + "_part"} for chrom, start, end in regions
        ],
    }
                ...
                for read in align.fetch(*reg):
                    ...
                    if read.is_paired:
                        if (
                            ...
                            or read.next_reference_start < reg[1]
                            or read.next_reference_end > reg[2]
                        ):
                            continue
...
def subset_domains(domain_file, regions, out_fn):
    ...
    for chrom, start, end in regions:
        sel = anno_df.query(
            f'chrom=="{chrom}" and chromStart>={start} and chromEnd <= {end}'
        ).copy()
    ...
    pd.write_csv(
        pd.concat(subset_df).rename({"chrom": "#chrom"}, axis=1), out_fn, seq="\t"
    )
